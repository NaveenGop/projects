...
def main():
    ...
    try:
        cursor.execute(
            r"""SELECT ProdCode, SKU FROM ##die GROUP BY ##die.ProdCode, ##die.SKU""")
        ...
        for show in cursor.fetchall():
            ...
            for row in curs:
                ...
                if tow[0][2] == tow[0][1] * 2:
                    ...
                    for bow in slursor.execute(r"""WITH TBL AS
                                        (
                                            SELECT
                                                Fact_EventPVR.ComponentEventId
                    ...
                    f.write(b"\t\t\t-Fewer CE.\n")
            print("Finish Die Count Time:", datetime.now())
            f.write(b"\tINVALID FIDS BY PROCESS STEP\n")
            for bow in slursor.execute(r"""SELECT
                                    ProcessStep
                                FROM
                                    ##die
                                GROUP BY
                ...
                f.write(f"\t\t{bow[0]}\n".encode())
                for slow in mursor.execute(fr"""SELECT DISTINCT
                                         Dim_DiePVR.CE
                                     ...
                                     GROUP BY
                                         ...
                                         , Dim_DiePVR.CE
                                     HAVING
                                         SUBSTRING(Fact_EventPVR.SerialNumber, 3, 2) = '{row[0]}'
                                         AND SUBSTRING(Fact_EventPVR.SerialNumber, 13, 4) = '{row[1]}'
                                         AND Dim_ProcessStep.ProcessStep = '{bow[0]}'
                                     ORDER BY
                                         CE"""):
                     f.write(f"\t\t\tCE {slow[0]} has an invalid FID in {slow[1]} drives.\n".encode())
            f.write(b"\n")
            print("Finish FID Time: ", datetime.now())
            f.write(b"\n")
